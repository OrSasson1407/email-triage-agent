...
// FetchUnread returns up to maxCount unread emails
func (c *Client) FetchUnread(ctx context.Context, maxCount int) ([]Email, error) {
	res, err := c.svc.Users.Messages.List(c.user).
		Q("is:unread").
		MaxResults(int64(maxCount)).
		Context(ctx).
		Do()
	if err != nil {
...
// FetchThread returns all messages in a thread (for context)
func (c *Client) FetchThread(ctx context.Context, threadID string) ([]Email, error) {
	res, err := c.svc.Users.Threads.Get(c.user, threadID).
		Format("full").
		Context(ctx).
		Do()
	if err != nil {
...
func (c *Client) fetchMessage(ctx context.Context, id string) (Email, error) {
	...
	for _, h := range msg.Payload.Headers {
		...
		case "Date":
			email.Date, _ = time.Parse(time.RFC1123Z, h.Value)
		}
	}
...
// MarkProcessed removes UNREAD label
...
// AddLabel adds a Gmail label to a message
func (c *Client) AddLabel(ctx context.Context, id, labelID string) error {
	_, err := c.svc.Users.Messages.Modify(c.user, id, &gmail.ModifyMessageRequest{
...
// CreateDraft saves a reply draft in Gmail
